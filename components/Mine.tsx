    ...
    const handleUpgrade = async () => {
        if (pointsBalance >= upgradeCost && !isLoading) {
            ...
            try {
                ...
                showToast('Wait for the next update.', 'error');
            } finally {
                setIsLoading(false);
            }
        }
                ...
                <div className="flex-grow mt-4 bg-[#f3ba2f] rounded-t-[48px] relative top-glow z-0">
                    <div className="mt-[2px] bg-[#1d2025] rounded-t-[46px] h-full overflow-y-auto no-scrollbar">
                        <div className="px-4 pt-1 pb-24">
                            <h1 className="text-2xl text-center mt-4">Current GMS Balance</h1>
                            ...
                            <div className="bg-[#272a2f] rounded-lg p-4 mt-6">
                                <div className="flex justify-between items-center mb-4">
                                    <p>Current gms:</p>
                                    <p className="text-[#44b75e]">{formatNumber(profitPerHour)}</p>
                                </div>
                                <div className="flex justify-between items-center mb-4">
                                    <p>Gms increase:</p>
                                    <p className="text-[#44b75e]">{formatNumber(upgradeCost)}</p>
                                </div>
                                <div className="flex justify-between items-center">
                                    <p>Harveted gms:</p>
                                    <p className="text-[#44b75e]">{formatNumber(upgradeIncrease)}</p>
                                </div>
                            </div>

                            <button
                                
                                disabled={pointsBalance < upgradeCost || pointsBalance > upgradeCost}
                                className={`w-full mt-6 py-3 rounded-lg text-center text-white font-bold ${pointsBalance >= upgradeCost && !isLoading ? 'bg-[#44b75e]' : 'bg-green-500 cursor-not-allowed'
                                    } relative`}
                            >
                                {isLoading ? (
                            ...
                            <div className="bg-[#272a2f] rounded-lg p-4 mt-6 flex items-start">
                                ...
                                Gms Tree seed phase is ongoing. <span className="text-white font-bold"> </span> After this phase, you will have a great opportunity to increase your Gms production!
                                </p>
                            </div>
