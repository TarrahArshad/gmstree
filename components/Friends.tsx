...
export default function Friends() {
  ...
  const handleCopyInviteLink = useCallback(() => {
    triggerHapticFeedback(window);
    navigator.clipboard
      .writeText(process.env.NEXT_PUBLIC_BOT_USERNAME ? `https://t.me/${process.env.NEXT_PUBLIC_BOT_USERNAME}/${process.env.NEXT_PUBLIC_APP_URL_SHORT_NAME}?startapp=gmsid_${getUserTelegramId(userTelegramInitData) || ""}` : "https://t.me/gmstree_community")
      .then(() => {
  ...
  const handleInviteFriend = useCallback(() => {
    ...
    const inviteLink = botUsername
      ? `https://t.me/${process.env.NEXT_PUBLIC_BOT_USERNAME}/${process.env.NEXT_PUBLIC_APP_URL_SHORT_NAME}?startapp=gmsid_${userTelegramId || ""}`
      : "https://t.me/gmstree_community";

    const shareText = `🎮 Join me in TonIce: Tap, Earn, and wait for the next Boom! 🏆\n🚀 Let's play and earn together!`;
            ...
            <div className="px-4 pt-1 pb-36">
              <div className="relative">
                ...
                <button
                  ...
                  className="block w-full mt-4 text-center text-green-500"
                >
                ...
                <div className="fixed bottom-20 left-1/2 transform -translate-x-1/2 w-full max-w-xl z-40 flex gap-4 px-4">
                  <button
                    className="flex-grow py-3 bg-green-500 rounded-lg text-white font-bold pulse-animation"
                    onClick={handleInviteFriend}
                  >
                    Invite friend
                  </button>
                  <button
                    className="w-12 h-12 bg-green-500 rounded-lg text-white font-bold flex items-center justify-center"
